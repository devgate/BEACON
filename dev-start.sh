...
if ! docker info >/dev/null 2>&1; then
    printf "❌ Docker가 실행되지 않았습니다.\n"
    printf "Docker Desktop을 시작하거나 Docker daemon을 실행해주세요.\n"
    printf "확인 방법: docker --version\n"
    exit 1
fi
printf "✅ Docker가 실행 중입니다.\n"

# AWS 환경변수 체크 및 설정
printf "🔑 AWS 자격증명 확인 중...\n"
...
if [ -f "$AWS_CREDS_FILE" ] && [ -z "$AWS_ACCESS_KEY_ID" ]; then
    printf "💾 저장된 AWS 자격증명을 찾았습니다.\n"
    source "$AWS_CREDS_FILE"
    printf "✅ AWS 자격증명을 로드했습니다.\n"
fi

# AWS 환경변수가 설정되어 있는지 확인
if [ -z "$AWS_ACCESS_KEY_ID" ] || [ -z "$AWS_SECRET_ACCESS_KEY" ]; then
    printf "⚠️  AWS 환경변수가 설정되지 않았습니다.\n"
    echo ""
    echo "다음 중 선택하세요:"
...
EOF
        ...
        printf "✅ AWS 자격증명이 설정되고 저장되었습니다.\n"
        printf "🔥 실제 AWS Bedrock 모드로 실행됩니다.\n"
        printf "💾 다음부터는 자동으로 로드됩니다.\n"
        printf "🗑️  삭제하려면: rm .aws-dev-credentials\n"
    else
        printf "📝 Mock 모드로 실행됩니다 (AWS 연결 없이 UI 개발 가능)\n"
    fi
else
    printf "✅ AWS 환경변수가 이미 설정되어 있습니다.\n"
    if [ -f "$AWS_CREDS_FILE" ]; then
        printf "💾 (저장된 자격증명에서 로드됨)\n"
    fi
    printf "🔥 실제 AWS Bedrock 모드로 실행됩니다.\n"
fi
...
echo ""
echo "⏹️  중지하려면: docker-compose -f docker-compose.dev.yml down"