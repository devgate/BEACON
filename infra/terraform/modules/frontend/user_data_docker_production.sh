...
# Create nginx configuration directory for ConfigMap support
echo "Setting up nginx configuration..."
mkdir -p /tmp/nginx-config

# Download and apply custom nginx configuration from local development
echo "Fetching custom nginx configuration..."
...
# Run BEACON Frontend Docker container with backend domain and ConfigMap nginx
echo "Starting BEACON Frontend Docker container with ConfigMap nginx..."
...
for attempt in {1..5}; do
  ...
  if ! docker ps --filter "name=beacon-frontend" --filter "status=running" --quiet | grep -q .; then
    ...
    # Restart with explicit environment variables and ConfigMap nginx
    echo "Restarting container with fresh environment and ConfigMap nginx..."
    docker run -d \
      --name beacon-frontend \
      --restart unless-stopped \
      -p 80:80 \
